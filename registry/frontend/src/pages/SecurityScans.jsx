...
export default function SecurityScans() {
  // For now, we'll fetch all agents and show their latest scans
  // In a production app, you'd have a dedicated endpoint for this
  const { data, isLoading, error } = useQuery({
    queryKey: ['agents-with-scans'],
  ...
  if (error) {
    return (
      <div className="bg-red-50 border border-red-200 text-red-700 px-4 py-3 rounded-md">
        Failed to load security scans: {error.message}
      </div>
    );
  }
            ...
            {agentList.map((agent) => (
              <tr key={agent.id} className="hover:bg-gray-50">
                ...
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                  {agent.last_security_scan_at
                    ? format(new Date(agent.last_security_scan_at), 'MMM d, yyyy HH:mm')
                    : 'Never'}
                </td>
